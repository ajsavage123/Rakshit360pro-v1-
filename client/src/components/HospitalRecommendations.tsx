...
import { MapPin, Phone, Clock, Star, Navigation, AlertCircle, Share2, Search, Stethoscope } from "lucide-react";
...
const HospitalRecommendations = ({ specialty, userLocation, summary }: HospitalRecommendationsProps) => {
  ...
  const handleCallHospital = (phone: string, hospitalName: string) => {
    if (phone && phone !== 'Phone not available') {
      // Create a confirmation dialog with patient summary
      const confirmCall = window.confirm(
        `Call ${hospitalName}?\n\nPatient Summary to mention:\n${summary || 'No medical summary available'}\n\nPhone: ${phone}`
      );
  ...
  const handleShareWhatsApp = (hospital: Hospital) => {
    // Create comprehensive patient summary for hospital
    const patientSummary = summary || 'No medical summary available';
    const hospitalInfo = `Hospital: ${hospital.name}\nAddress: ${hospital.address}\nPhone: ${hospital.phone || 'Not available'}`;
    
    const fullMessage = `🏥 PATIENT REFERRAL
...
⭐ Rating: ${hospital.rating ? hospital.rating + '/5' : 'Not rated'}

Sent via Medical AI Assistant`;

    if (hospital.phone && hospital.phone !== 'Phone not available') {
      // Clean phone number - remove all non-digit characters except +
      const cleanPhone = hospital.phone.replace(/[^\d+]/g, '');
      // If it doesn't start with +, assume it needs country code
      const phoneNumber = cleanPhone.startsWith('+') ? cleanPhone : `+1${cleanPhone.replace(/^\+?1?/, '')}`;
                  ...
                  <CardContent className="p-4 sm:p-6 lg:p-8">
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                      ...
                      >
                        ...
                        {hospital.phone && hospital.phone !== 'Phone not available' ? 'Call Hospital' : 'No Phone'}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
